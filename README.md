...
> 首次启动时，应用自动在 `notemaster-data/notemaster.db` 创建数据库，并预置 **5 篇示例笔记** 和 **3 个默认分类**（工作 / 学习 / 生活）。
...
> 右键文件夹：**新建子文件夹 / 重命名 / 收藏 / 导出 Markdown / 删除**
> 
> 支持拖拽笔记到分类、拖拽文件夹调整层级
...
![数据统计](screenshots/08-stats.png)

> 总笔记数、今日/本月新增、分类分布饼图、月度趋势柱状图

### 全局搜索

![全局搜索](screenshots/09-search-dialog.png)

> `Ctrl+Shift+F` 唤起，输入即搜，搜索历史自动保存

### 回收站

![回收站](screenshots/10-trash.png)

> 软删除机制，支持恢复或彻底删除；批量模式可多选删除
...
- **导出分类**：右键文件夹 → **「导出 .md」** → 该分类下所有笔记合并为一个 Markdown 文件
...
本地 SQLite 数据库文件：`notemaster-data/notemaster.db`
